...
1.- This method first ranks all $N$ genes in our dataset by their decreasing association with the studied phenotype/treatment. 
...
## Gene Set Enrichment Analysis (GSEA) - Method  
2.- The $N_S$ genes in the pathway $S$ are located along the ranking: if the pathway genes are up- or down-regulated, then they appear at the top or bottom of the ranking, respectively.
...
## Gene Set Enrichment Analysis (GSEA) - Method  
3.- GSEA calculates an enrichment score based on the ranking: walk throughout the ranking and, starting from 0, add $\frac{|logFC_g|}{\sum_{g \in S}|logFC_g|}$ if the gene is in the pathway $S$ (go ↑ 1 step of that size) or $-\frac{1}{N-N_S}$ if not (go ↓ 1 step of that size). 
...
* The increase $\frac{|logFC_g|}{\sum_{g \in S}|logFC_g|}$ in the enrichment score for those genes in $S$ is proportional to their $logFC$ as we want to capture not only if the pathway gene is at the top/bottom but also to what extent it is affected. This number is normalized by the sum of the $logFC$ of all genes in $S$ to account for the size of the pathway and the scale of the metric used for ranking.

* The decrease $\frac{1}{N-N_S}$ for those genes not in $S$ represents the same decrease in the enrichment score, normalized by the number of such genes outside $S$ (i.e. $N-N_S$ genes).

## Gene Set Enrichment Analysis (GSEA) - Method  

The final enrichment score $ES$ for pathway $S$ represents the value in the walking sum that more greatly deviates from zero.
...
## Gene Set Enrichment Analysis (GSEA) - Method  
4.- The p-value of the enrichment score is computed empirically by either: 1) permuting the sample phenotype/treatment labels, performing differential gene expression again to obtain new gene logFCs, then re-computing $ES$ for $S$, or 2) drawing random gene sets of the same size as $S$ from the genome and computing their enrichment score against the original ranking.

Both generate an empirical null distribution of enrichment scores. The p-value is the proportion of such  scores that are equal or greater than the observed $ES$.
