...
1.- This method first ranks all genes in our dataset by their decreasing association with the studied phenotype/treatment. 
...
## Gene Set Enrichment Analysis (GSEA) - Method  
2.- The genes in the pathway are located along the ranking: if the pathway genes are up- or down-regulated, then they appear at the top or bottom of the ranking, respectively.
...
## Gene Set Enrichment Analysis (GSEA) - Method  
3.- GSEA calculates an enrichment score based on the ranking:  walk throughout the ranking and, starting from 0, go up 1 step if the gene is in the pathway or go down 1 step if not.
...
The final enrichment score for the pathway represents the value in the walking sum that more greatly deviates from zero.
...
## Gene Set Enrichment Analysis (GSEA) - Method  
4.- The p-value of the enrichment score is computed by either: 1) permuting the sample phenotype/treatment labels to obtain new gene logFCs and with it a new enrichment score, or 2) drawing random gene sets of the same size as the pathway and computing their enrichment score against the original ranking.

Both generate a distribution of enrichment scores without biological meaning. The p-value is the proportion of such scores that are equal or greater than the observed score.
